const addElement = (e, node = 'div', txt = '', attr = '', value = '') => {
    e.preventDefault();
...
const searchElements = (e, nameElement) => {
    ...
    if (elements.length) {
        ...
        infoElement.innerHTML = `<p class="result__number-info">Nie znalazłem <strong>${nameElement}</strong></p>`
    }
}
...
const showInfo = (elements, infoElement) => {
    elements.forEach((element) => {
        ...
        item.className = 'result__element-description element-info';
        item.innerHTML = `
